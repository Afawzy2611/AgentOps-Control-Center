...
from app.external_evidence import AuthorizationDecision, EvidencePolicy, EvidenceStatus, ExternalEvidence, RetryPolicy, authorize_operation, bounded_text, execute_evidence_operation


def test_evidence_envelope_contains_required_metadata():
    evidence = ExternalEvidence.success(provider="airbyte", connector="github", operation="search_issues", payload={"items": [1]}, provenance={"source": "github"}, retry_count=1)
...
def test_transient_failures_are_retried_with_a_bounded_count():
    ...
    policy = EvidencePolicy({("airbyte", "github", "read"): True})
    evidence, audit = execute_evidence_operation(provider, policy, "github", "read", {}, RetryPolicy(max_retries=2), max_output_chars=4)
...
def test_denied_operation_is_not_retried():
    provider = FakeProvider()
    evidence, audit = execute_evidence_operation(provider, EvidencePolicy({}), "github", "read", {}, RetryPolicy(max_retries=2), max_output_chars=100)
...
def test_audit_event_never_contains_secret_values():
    provider = FakeProvider()
    policy = EvidencePolicy({("airbyte", "github", "read"): True})
    _, audit = execute_evidence_operation(provider, policy, "github", "read", {"token": "SECRET_TOKEN"}, RetryPolicy(max_retries=0), max_output_chars=100)
...
class FakeConnector:
    ...
    def execute(self, entity, action, params=None):
        return {"entity": entity, "action": action, "params": params or {}, "items": []}


def test_airbyte_adapter_exposes_only_allowlisted_read_operations():
    from app.airbyte_provider import AirbyteEvidenceProvider

    provider = AirbyteEvidenceProvider(EvidencePolicy({("airbyte", "github", "inspect_connector"): True, ("airbyte", "github", "read_skill_docs"): True, ("airbyte", "github", "execute"): True}), connector_factory=lambda name: FakeConnector(), sdk_available=True)
    assert provider.inspect_capability("github")["connector"] == "github"
    evidence = provider.execute("github", "inspect_connector", {})
    assert evidence.status is EvidenceStatus.SUCCESS
    evidence = provider.execute("github", "execute", {"entity": "issues", "action": "search", "params": {"q": "bug"}})
    assert evidence.status is EvidenceStatus.SUCCESS


def test_airbyte_adapter_rejects_unknown_operation():
    from app.airbyte_provider import AirbyteEvidenceProvider
    provider = AirbyteEvidenceProvider(EvidencePolicy({("airbyte", "github", "execute"): True}), connector_factory=lambda name: SimpleNamespace(), sdk_available=True)
    evidence = provider.execute("github", "delete_repository", {})
    assert evidence.status is EvidenceStatus.FAILED
    assert evidence.error == "Unsupported external operation"


def test_airbyte_adapter_rejects_write_action_even_when_execute_is_allowlisted():
    from app.airbyte_provider import AirbyteEvidenceProvider
    provider = AirbyteEvidenceProvider(EvidencePolicy({("airbyte", "github", "execute"): True}), connector_factory=lambda name: FakeConnector(), sdk_available=True)
    evidence = provider.execute("github", "execute", {"entity": "issues", "action": "create", "params": {}})
    assert evidence.status is EvidenceStatus.FAILED
    assert evidence.error == "Connector action is not read-only"
